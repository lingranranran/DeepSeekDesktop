...
> **当前状态**：M0–M4 已完成；当前版本 v0.2.0。v0.2.0 起支持自动更新（GitHub Releases + blockmap 差量下载）与 tag 触发的自动发布流水线；内置运行时以单文件 tar 分发：安装秒级完成，首次启动时解压到用户数据目录并显示进度。
...
日志文件位于 Electron `userData/logs/` 目录（设置页提供"打开日志目录"入口），包含主进程事件与 dsh 的 stdout/stderr 转发。
...
| M6 运行时独立升级 | dsh 版本与应用发版解耦：清单发现 + 下载校验 + 备份回滚 | — | 规划中 |
| M7 桌面体验深化 | 任务栏进度、通知定位会话、深度链接、多窗口 | — | 规划中 |
